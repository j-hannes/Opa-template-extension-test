...
Oxml = {{
  ...
  tags = {{
     ...
     * Headings [complete]
     */
    heading(level, content) = match level
      "1" -> <h1>{content}</h1>
      "2" -> <h2>{content}</h2>
      "3" -> <h3>{content}</h3>
      "4" -> <h4>{content}</h4>
  ...
  parse2xhtml(content:string):xhtml =
    Template.parse(oxml_engine, content)
    |> Template.to_xhtml(oxml_engine, _)
  ...
  rec val oxml_engine = {Template.empty with
    ...
    export(extension, _) =
      match Template.from_extension(extension)
  ...
  extend(xmlns):Template.content =
    html = convert_to_string(xmlns.content) |> parse2xhtml
    tag  = match xmlns.tag "container" -> {container}
                           "heading"   -> {heading}
                           "ilabel"    -> {label}
                           "ibutton"   -> {button}
                           _           -> {unsupported tag=xmlns.tag}          
    Template.to_extension({~tag args=xmlns.args content=html})
  ...
  get_attribute(att_name:string, att_list:list(Xml.attribute)):string =
    match List.find(a -> a.name == att_name, att_list)
      {some = {~value ...}} -> value
      {none}                -> ""
